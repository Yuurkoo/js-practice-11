...
function getIntersection(arr1, arr2) {
  ...
  const intersectArr1 = new Set(arr1);
  const intersectArr2 = new Set(arr2);

  const intersectionSet = new Set();
  for (const elem of intersectArr1) {
    if (intersectArr2.has(elem)) {
...
function iterateSet(set) {
  ...
  const iterSet = new Set(set);

  const keySet = iterSet.keys();
  for (const key of keySet) {
    console.log(key);
  }

  const valueSet = iterSet.values();
  for (const value of valueSet) {
    console.log(value);
  }

  const entriesSet = iterSet.entries();
